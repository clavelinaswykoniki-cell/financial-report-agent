...
- 2026-06-24 dashboard layout reorder is live: the old top-left current-a summary block is removed, the connected four-card market strip is now the first content below the title/actions, and the historical curve / a-Kline section sits directly below the four-card strip.
- Layout reorder release: branch `codex/511130-a-monitor`, layout commit `6b29e98`, production URL `https://511130-live-monitor-production.up.railway.app`; use the latest same-branch `SUCCESS` item from `railway deployment list --json` as the current deployment ID.
- Verification for the layout reorder passed: `python3.12 -m py_compile scripts/511130_live_monitor/live_a_dashboard.py tests/test_511130_live_monitor.py`; `python3.12 -m unittest tests.test_511130_live_monitor` returned 91 tests OK; local Chrome screenshot on `127.0.0.1:8811` returned `cards=4`, `hasPrimary=false`, `hasLatestA=false`, and `chartBelowQuote=true`; production smoke returned `ok=true`, `issues=[]`, `process_ok=true`, `auto_loop=running`; production HTML returned `refreshSec3=true`, `quote_before_chart=true`, `has_primary_value=false`, `has_latest_a=false`, and `chart_title=true`.
- Operational note: Railway has `/data` volume mounted and the auto loop runs in the Railway process, so production does not depend on local Codex being online. `/health.public_readonly=false`, so public write endpoints remain enabled unless Railway is changed to `A_MONITOR_PUBLIC_READONLY=1`.
